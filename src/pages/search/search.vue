<template>
  <div class="w-100% h-100% flex flex-col items-stretch justify-between">
    <!-- app bar -->
    <var-app-bar title="搜索源"> </var-app-bar>
    ...
    <div class="flex-1 overflow-y-auto" :key="refreshKey">
      <var-loading description="LOADING" :loading="loading" class="h-100% w-100%">
        ...
        <template v-else>
          <var-result class="h-100%" type="empty" image-size="30" description="结果为空哦~"> </var-result>
        </template>
      </var-loading>
    </div>
  </div>
...
const searchFn = () => {
  loading.value = true;
  return axios
    .post(`http://foodieguide.com/iptvsearch/?page=1&s=${serchValue.value}`)
    .then(res => {
